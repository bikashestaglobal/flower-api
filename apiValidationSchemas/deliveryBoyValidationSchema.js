...
// createDeliveryBoy
module.exports.createDeliveryBoy = Joi.object({
...
// loginDeliveryBoy
module.exports.loginDeliveryBoy = Joi.object({
  email: Joi.string().email().trim().required(),
  password: Joi.string()
    .regex(
      /^(?=.*\d)(?=.*[A-Z])(?=.*[a-z])(?=.*[a-zA-Z!#$%&@? "])[a-zA-Z0-9!#$%&@?]{6,20}$/
...
// getDeliveryBoyById
module.exports.getDeliveryBoyById = Joi.object({
  id: Joi.custom(customCallback),
});

// getAllDeliveryBoys
module.exports.getAllDeliveryBoys = Joi.object({
  skip: Joi.string(),
  limit: Joi.string(),
...
// updateDeliveryBoy
module.exports.updateDeliveryBoy = Joi.object({
  name: Joi.string().trim(),
  email: Joi.string().email().trim(),
  mobile: Joi.string()
    .regex(/^[6-9]\d{9}$/)
...
// assignPincodes
module.exports.assignPincodes = Joi.object({
  pincodes: Joi.array().items(Joi.string().custom(customCallback)),
});
